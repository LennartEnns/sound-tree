...
class LEDController:
    def __init__(self):
        self.ser = Serial("/dev/cu.usbserial-1110", 1000000)
        self.ser.readline() # Read line (wait for ready)

    def send_all(self, hex_array): # array contains elements of 3 bytes
        for i in range(NUM_LEDS):
            for j in range(3):
                sendChar = hex_array[i][j]
                self.ser.write(sendChar)
    def blink(self, color, n_times, t_after_on = 0.3, t_after_off = 0.4):
        hex_color = rgb_to_bytes(color)
